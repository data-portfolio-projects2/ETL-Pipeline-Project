...
from validate import DataValidator as validate
from extract import DataExtractor as extract
from ingest import DataIngestor as ingest
from save import DataSaver as save
from load import Loader as load
...
class Downloader(metaclass=static):
    ...
    def download_data():
        download.download_data() # This will be optimized for scalability (handling large datasets)


class Transformer(metaclass=static):

    def extract_data():
        df = extract.extract_data() # This will be optimized for scalability (handling large datasets)
        return df
...
class Ingestor(metaclass=static):
    
    def ingest():
        return ingest.query_table()
    
    def load_to_postgres(df):
        return ingest.load_to_postgres(df)
